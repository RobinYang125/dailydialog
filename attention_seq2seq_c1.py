...
class DataLoader(object):
    ...
    def __init__(self, is_toy=False):
        if is_toy:
            ...
            self.source_train = ['data_dom/c1/data_train.txt']
            self.source_test = ['data_dom/c1/data_test.txt']
            self.batch_size = BATCH_SIZE
            self.max_sequence_length = MAX_SEQUENCE_LENGTH
            self.source_validation = ['data_dom/c1/data_val.txt']
...
class Seq2seq(object):
    ...
    def create_model(self):
        ...
        with tf.variable_scope("decoder") as decoder:
            ...
            attention_mechanism = tf.contrib.seq2seq.BahdanauAttention(self.embedding_size, self.encoder_outputs)
            ...
            training_decoder = tf.contrib.seq2seq.BasicDecoder(decoder_cell,
                                                               ...
                                                               decoder_cell.zero_state(
                                                                   tf.shape(self.encoder_outputs)[0], tf.float32
                                                               ),
                                                               output_layer)
